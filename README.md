...
**Важно:** После запуска команды приложению требуется некоторое время для инициализации. Пожалуйста, подождите несколько секунд перед тем, как интерфейс полностью отобразится в браузере. Это время необходимо для загрузки модели машинного обучения и подготовки всех компонентов системы.
